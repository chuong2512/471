...
public class ScrollListManager : MonoBehaviour
{
    ...
    private void LoadAllTexture()
    {
        for (int i = 0; i < transform.childCount; i++)
        {
            ...
            loadPixels = LoadImage(saveIndexString + i.ToString());
            ...
            if (loadPixels != null)
            {
                ...
                transform.GetChild(i).GetComponent<Image>().sprite = Sprite.Create(tex, new Rect(0, 0, texWidth, texHeight), Vector2.zero, 100);
            }
        }
    }
